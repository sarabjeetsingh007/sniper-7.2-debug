...
{
   //Sarabjeet: [Print Cache Activity] File close
   if(trace_CachelineActivity)
   {
      traceactivityL1I.close();
      traceactivityL1D.close();
      traceactivityL2.close();
      traceactivityL3.close();
   }

	//Sarabjeet: [Print Cache Writes - Data] File close
   if(trace_WriteData)
   {
      tracewriteL1I.close();
      tracewriteL1D.close();
   ...
   m_shmem_perf_model(shmem_perf_model)
{
   ...
   //Sarabjeet: [Print Cache Activity] File open
   if(trace_CachelineActivity)
   {
      if(core_id==0 && m_mem_component==2)
      {
           traceactivityL1I.open("traceactivityL1I.log", std::ios::out );
           traceactivityL1D.open("traceactivityL1D.log", std::ios::out );
           traceactivityL2.open("traceactivityL2.log", std::ios::out );
           traceactivityL3.open("traceactivityL3.log", std::ios::out );
      }
   }

	//Sarabjeet: [Print Cache Writes - Data] File open
   if(trace_WriteData)
   {
   	if(core_id==0 && m_mem_component==2)
   	{
   		  tracewriteL1I.open("tracewriteL1I.log", std::ios::out );
   		  tracewriteL1D.open("tracewriteL1D.log", std::ios::out );
   		  tracewriteL2.open("tracewriteL2.log", std::ios::out );
   		  tracewriteL3.open("tracewriteL3.log", std::ios::out );
   	}
   }
...
/*****************************************************************************
 * sarab: called by L1 (on miss). Copies data from higher level to L1, so that accessCache() can access from L1
 *****************************************************************************/
...
/*****************************************************************************
 * sarab: Bring Block of Data from higher levels to L1
 *****************************************************************************/
...
/*****************************************************************************
 * sarab: update achieved by retrieveCacheBlock followed by writeCacheBlock
 *****************************************************************************/
...
/*****************************************************************************
 * sarab: On STORE operation, writes the block on the next levels of cache, if write-through
 *****************************************************************************/
